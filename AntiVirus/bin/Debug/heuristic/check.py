...
def upload_file(filePath):
        ...
        features = pickle.loads(
            open(os.path.join(os.path.dirname(os.path.realpath(__file__)), 'classifier/features.pkl'), 'rb').read())
...
if __name__ == '__main__':
    ...
    mylist = []
    with open("list.txt", 'r', encoding="utf8", newline="") as f:
            lines = f.readlines()
            for line in lines:
                try:
                    updatePath = line.replace("\\", "/")
                    newPath = updatePath.replace("\n", "")
                    updateNewPath = newPath.replace("\r", "")
                    upload_file(updateNewPath)
                    mylist.append(prediction)
                except:
                    print("Analyze PEFile error")
